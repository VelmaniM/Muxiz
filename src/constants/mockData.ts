export interface Track {
  id: string;
  title: string;
  artist: string;
  album: string;
  artwork: string;
  audioUrl: string;
  duration: number; // in seconds
  genre: string;
  isLiked?: boolean;
  lyrics?: string[];
  gradient: [string, string];
}

export interface Playlist {
  id: string;
  title: string;
  subtitle: string;
  cover: string;
  tracks: Track[];
  followers: string;
}

// Preloaded Songs with 100% Official Movie Poster / Studio Artwork (No stock playlist photos!)
export const INITIAL_PRELOADED_TRACKS: Track[] = [
  {
    id: 'track_1',
    title: 'Aadi Pona Aavani',
    artist: 'Gana Bala',
    album: 'Attakathi',
    artwork: 'https://c.saavncdn.com/264/Attakathi-Tamil-2012-500x500.jpg',
    audioUrl: 'https://cdn.pixabay.com/download/audio/2022/05/27/audio_1808fbf07a.mp3?filename=melody-of-nature-112702.mp3',
    duration: 281,
    genre: 'Gana',
    gradient: ['#1DB954', '#0B0C10'],
  },
  {
    id: 'track_2',
    title: 'Adatha Attamellam',
    artist: 'Karthik',
    album: 'Surya Vamsam',
    artwork: 'https://c.saavncdn.com/398/Suryavamsam-Tamil-1997-500x500.jpg',
    audioUrl: 'https://cdn.pixabay.com/download/audio/2022/03/15/audio_c8c8a73467.mp3?filename=summer-walk-15651.mp3',
    duration: 271,
    genre: 'Melody',
    gradient: ['#1DB954', '#0B0C10'],
  },
  {
    id: 'track_3',
    title: 'Aaathi Adi Aaathi',
    artist: 'Sadhika K R',
    album: 'Single',
    artwork: 'https://c.saavncdn.com/712/Aaathi-Adi-Aaathi-Tamil-2023-500x500.jpg',
    audioUrl: 'https://cdn.pixabay.com/download/audio/2022/01/18/audio_d0a13f69d2.mp3?filename=chill-abstract-intention-12099.mp3',
    duration: 164,
    genre: 'Folk',
    gradient: ['#1DB954', '#0B0C10'],
  },
  {
    id: 'track_4',
    title: 'A Love Blossoms',
    artist: 'G.V. Prakash Kumar & Navin Iyer',
    album: 'Raja Rani',
    artwork: 'https://c.saavncdn.com/780/Raja-Rani-Tamil-2013-500x500.jpg',
    audioUrl: 'https://cdn.pixabay.com/download/audio/2022/10/14/audio_9939f7f458.mp3?filename=lofi-study-112191.mp3',
    duration: 88,
    genre: 'Instrumental',
    gradient: ['#1DB954', '#0B0C10'],
  },
  {
    id: 'track_5',
    title: 'A Life Full of Love Theme',
    artist: 'Anirudh Ravichander & The Chennai Strings',
    album: '3',
    artwork: 'https://c.saavncdn.com/495/3-Tamil-2011-500x500.jpg',
    audioUrl: 'https://cdn.pixabay.com/download/audio/2022/05/16/audio_db6591201e.mp3?filename=ambient-piano-10781.mp3',
    duration: 116,
    genre: 'Theme',
    gradient: ['#1DB954', '#0B0C10'],
  },
  {
    id: 'track_6',
    title: '200 Goats',
    artist: 'G.V. Prakash Kumar',
    album: 'Captain Miller',
    artwork: 'https://c.saavncdn.com/023/Captain-Miller-Tamil-2024-500x500.jpg',
    audioUrl: 'https://cdn.pixabay.com/download/audio/2022/03/10/audio_c26569116e.mp3?filename=classic-hip-hop-15941.mp3',
    duration: 280,
    genre: 'Soundtrack',
    gradient: ['#1DB954', '#0B0C10'],
  },
];

export const MOCK_TRACKS: Track[] = INITIAL_PRELOADED_TRACKS;

// Dedicated Playlists - Populated with official movie posters
export const MOCK_PLAYLISTS: Playlist[] = [
  {
    id: 'pl_newly_added',
    title: 'Newly Added',
    subtitle: 'Uploaded tracks & latest cloud additions',
    cover: 'https://c.saavncdn.com/264/Attakathi-Tamil-2012-500x500.jpg',
    tracks: INITIAL_PRELOADED_TRACKS,
    followers: 'Updated live from Cloud DB',
  },
  {
    id: 'pl_sai_abhyankkar',
    title: 'Sai Abhyankkar Mix',
    subtitle: 'Sai Abhyankkar, Anirudh Ravichander, G.V. Prakash',
    cover: 'https://c.saavncdn.com/712/Aaathi-Adi-Aaathi-Tamil-2023-500x500.jpg',
    tracks: INITIAL_PRELOADED_TRACKS,
    followers: 'Database Tracks Only',
  },
  {
    id: 'pl_paruthiveeran',
    title: 'Paruthiveeran & Rural Folk',
    subtitle: 'Yuvan Shankar Raja, Ilaiyaraaja, Karthik Raja',
    cover: 'https://c.saavncdn.com/398/Suryavamsam-Tamil-1997-500x500.jpg',
    tracks: INITIAL_PRELOADED_TRACKS,
    followers: 'Database Tracks Only',
  },
  {
    id: 'pl_trending_tamil',
    title: 'Trending Now Tamil',
    subtitle: 'Anirudh, G.V. Prakash, A.R. Rahman',
    cover: 'https://c.saavncdn.com/780/Raja-Rani-Tamil-2013-500x500.jpg',
    tracks: INITIAL_PRELOADED_TRACKS,
    followers: 'Database Tracks Only',
  },
  {
    id: 'pl_dc_soundtrack',
    title: 'DC Motion Picture Soundtrack',
    subtitle: 'Hans Zimmer, Danny Elfman, Junkie XL',
    cover: 'https://c.saavncdn.com/023/Captain-Miller-Tamil-2024-500x500.jpg',
    tracks: INITIAL_PRELOADED_TRACKS,
    followers: 'Database Tracks Only',
  },
  {
    id: 'pl_goindhamma',
    title: 'Goindhamma Hits',
    subtitle: 'Hiphop Tamizha, Anirudh, Santhosh Narayanan',
    cover: 'https://c.saavncdn.com/495/3-Tamil-2011-500x500.jpg',
    tracks: INITIAL_PRELOADED_TRACKS,
    followers: 'Database Tracks Only',
  },
  {
    id: 'pl_pakka_local',
    title: 'Pakka Local Mass Beats',
    subtitle: 'Anirudh Ravichander, G.V. Prakash',
    cover: 'https://c.saavncdn.com/264/Attakathi-Tamil-2012-500x500.jpg',
    tracks: INITIAL_PRELOADED_TRACKS,
    followers: 'Database Tracks Only',
  },
  {
    id: 'pl_sajanka',
    title: 'Sajanka Melodies',
    subtitle: 'Harris Jayaraj, A.R. Rahman, Yuvan',
    cover: 'https://c.saavncdn.com/780/Raja-Rani-Tamil-2013-500x500.jpg',
    tracks: INITIAL_PRELOADED_TRACKS,
    followers: 'Database Tracks Only',
  },
];

export const MOCK_GENRES = [
  { id: 'g1', name: 'Gana & Rural', color: '#8D67AB', cover: 'https://c.saavncdn.com/264/Attakathi-Tamil-2012-500x500.jpg' },
  { id: 'g2', name: 'Tamil Hits', color: '#E8115B', cover: 'https://c.saavncdn.com/712/Aaathi-Adi-Aaathi-Tamil-2023-500x500.jpg' },
  { id: 'g3', name: 'Melodies', color: '#BA5D07', cover: 'https://c.saavncdn.com/398/Suryavamsam-Tamil-1997-500x500.jpg' },
  { id: 'g4', name: 'Rock & Beats', color: '#E91429', cover: 'https://c.saavncdn.com/780/Raja-Rani-Tamil-2013-500x500.jpg' },
  { id: 'g5', name: 'Folk & Culture', color: '#608108', cover: 'https://c.saavncdn.com/495/3-Tamil-2011-500x500.jpg' },
  { id: 'g6', name: 'Instrumental', color: '#DC5D9B', cover: 'https://c.saavncdn.com/023/Captain-Miller-Tamil-2024-500x500.jpg' },
];
